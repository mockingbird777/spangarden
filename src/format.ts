...
function terminal(report: AnalysisReport): string {
  ...
  for (const row of report.usage) {
    ...
    lines.push(`  ${row.type.padEnd(5)}  ${singleLine(row.name).padEnd(24).slice(0, 24)}  ${String(row.calls).padStart(3)} calls  ${duration(row.durationMs).padStart(8)}  ${row.errors} err${cost}`);
  }
  if (report.loops.length > 0) {
    lines.push("", "  LOOP SIGNALS");
    for (const loop of report.loops.slice(0, 10)) lines.push(`  ${loop.reason}  ${singleLine(loop.signature)}  [${loop.spanIds.map(singleLine).join(" → ")}]`);
  }
...
function markdownCode(value: string | number): string {
  const content = singleLine(String(value));
  const longest = Math.max(0, ...[...content.matchAll(/`+/gu)].map((match) => match[0].length));
  const delimiter = "`".repeat(longest + 1);
  const padding = content.startsWith("`") || content.endsWith("`") || content.startsWith(" ") || content.endsWith(" ") ? " " : "";
...
function markdown(report: AnalysisReport): string {
  ...
  if (report.loops.length > 0) {
    lines.push("", "## Loop signals", "");
    for (const loop of report.loops) lines.push(`- **${loop.reason}:** ${markdownCode(loop.signature)} — ${loop.spanIds.map((id) => markdownCode(id)).join(" → ")}`);
  }
